  ...
  return (
    <div className="relative min-h-screen w-full flex items-center justify-start overflow-hidden font-sans">
      ...
      >
        ...
        <motion.p
          variants={itemVariants}
          className="mt-8 text-base sm:text-lg text-zinc-400 max-w-sm font-normal leading-relaxed"
        >
          Your central hub for presence and productivity. Access your workspace
          stats from anywhere, and verify your location to clock into the daily
          stream.
        </motion.p>

        {/* Action Buttons with React Router Navigation */}
        <motion.div
          variants={itemVariants}
          className="mt-12 w-full flex flex-row items-center gap-4 max-w-xs"
        >
          {/* Join Now - Navigates to SignUp */}
          <button
            onClick={() => navigate("/signup")}
            className="group flex-1 flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-sm py-4 px-4 rounded-xl transition-all active:scale-95 shadow-lg cursor-pointer"
          >
        ...
        >
          Full attendance features require on-site verification
        </motion.p>
      </motion.div>
    </div>
  );
