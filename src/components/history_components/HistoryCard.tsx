import { motion } from "framer-motion";
import { ChevronRight, FileText, Timer, MapPin } from "lucide-react";
import type { WorkspaceHistoryItem } from "../../types/auth.types";

interface HistoryCardProps {
  log: WorkspaceHistoryItem;
  onInspectClick: (log: WorkspaceHistoryItem) => void;
}

export default function HistoryCard({ log, onInspectClick }: HistoryCardProps) {
  // Use log properties matching your AuthContext/Drizzle interface
  return (
    <motion.div
      initial={{ opacity: 0, y: 8 }}
      animate={{ opacity: 1, y: 0 }}
      className="w-full bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-5 rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-4 transition-all hover:border-zinc-300 dark:hover:border-zinc-700 hover:shadow-xs text-left"
    >
      <div className="space-y-2.5 flex-1 min-w-0">
        <div className="flex flex-wrap items-center gap-2 text-[11px] font-black tracking-wider uppercase">
          <span className="text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/20 px-2.5 py-1 rounded-md">
            {log.dayName}
          </span>
          <span className="text-zinc-400 dark:text-zinc-500 font-bold">
            {log.logDate}
          </span>

          {/* Logic check: Ensure these fields exist in WorkspaceHistoryItem */}
          <span
            className={`inline-flex items-center gap-1 font-bold ${log.isLate ? "text-amber-600" : "text-emerald-600"}`}
          >
            <Timer className="w-3 h-3" />
            {log.isLate ? "Late" : "On Time"}
          </span>

          <span
            className={`inline-flex items-center gap-1 font-bold ${log.isOnSite ? "text-blue-600" : "text-purple-600"}`}
          >
            <MapPin className="w-3 h-3" />
            {log.isOnSite ? "On Site" : "Remote"}
          </span>
        </div>

        <div className="space-y-1">
          <h3 className="text-sm font-bold text-zinc-800 dark:text-zinc-100 truncate">
            {log.isLogEmpty ? (
              <span className="text-zinc-400 dark:text-zinc-500 font-normal italic">
                No work details submitted.
              </span>
            ) : (
              log.projectTitle
            )}
          </h3>
          {!log.isLogEmpty && (
            <p className="text-xs text-zinc-500 dark:text-zinc-400 font-medium line-clamp-1">
              {log.projectDescription}
            </p>
          )}
        </div>
      </div>

      <button
        onClick={() => onInspectClick(log)}
        className="flex items-center justify-center gap-1.5 text-xs font-black px-4 py-2.5 bg-zinc-50 dark:bg-zinc-800/50 border border-zinc-200 rounded-xl hover:bg-zinc-100 transition-all self-start sm:self-center shrink-0"
      >
        <FileText className="w-3.5 h-3.5 text-zinc-500" />
        <span>View Details</span>
        <ChevronRight className="w-3.5 h-3.5" />
      </button>
    </motion.div>
  );
}
