...
export const DesignDetailView = ({ data }: DesignDetailViewProps) => {
  const designTools =
    data.workData?.tools?.length > 0
      ? data.workData.tools
      : data.workData?.stacks || [];
  ...
  return (
    <div className="space-y-4 pt-4 border-t border-zinc-100 dark:border-zinc-800">
      ...
      <div className="flex flex-col gap-2">
        ...
        {/* Assets / Live Handover Link - Using the confirmed 'liveUrl' key */}
        {(data.workData?.liveUrl || data.workData?.assetsUrl) && (
          <a
            href={data.workData.liveUrl || data.workData.assetsUrl}
            target="_blank"
            rel="noreferrer"
